...
class CacheCityEvents
  ...
  def self.perform()
    ...
    world_events = events.sort_by{|event| event.result_order_score(nil, [0,0])}.reverse[0..100]
    world_event_ids = world_events.map{|event| event.id}
    max_world_reactions = events.map{|event| event.n_reactions}.max
    $redis.set("HEAT_WORLD_MAX", max_world_reactions)

    $redis.multi do
      $redis.del("#{city_key}_EXP_LIST")
      world_event_ids.each {|event_id| $redis.sadd("#{city_key}_EXP_LIST", event_id)}
    end
