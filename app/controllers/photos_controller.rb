class PhotosController < ApplicationController
  ...
  def index
    ...
    last_venues_id = Photo.where(:time_taken.gt => 3.hours.ago.to_i).excludes(status: "novenue").distinct(:venue_id) #all photos from last 3hours that have venues
    last_venues = {}
    last_venues_id.each do |venue_id|
      last_venues[venue_id] = Photo.where(:venue_id => venue_id).where(:time_taken.gt => 3.hours.ago.to_i).distinct(:user_id).count 
    end
