...
# there has to be a better way of doing this that doesn't require creating a time object
  ...
  def self.get_localized_results(lon_lat, max_dist, options={})
    ...
    if options[:facebook_user]
      ...
      events[0..(num_events -1)].each {|event| already_personalized << event.id unless event.personalize_for[facebook_user.now_id].nil?}
      ...
      if (already_personalized + personalized_events.count) < 5
        ...
        personalized_events.push(*(more_events[0..(5 - (already_personalized + personalized_events).count)]))
      end
